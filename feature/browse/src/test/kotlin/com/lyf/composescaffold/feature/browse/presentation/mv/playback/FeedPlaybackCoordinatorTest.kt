...
/** 播放协调器的音乐与 MV 切换、暂停原因和检查点语义测试。 */
@OptIn(ExperimentalCoroutinesApi::class)
class FeedPlaybackCoordinatorTest {

    /** 记录播放、拖动与重试调用、状态由测试手动推送的播放器替身。 */
    ...
    /** 全局播放器替身：只记录调用不产生音频，playTrack/pause/resume 递增 intentVersion 贴合真实乐观锁。 */
    ...
    /** MV 退后台延时释放，宽限期内回前台可无缝恢复，超时才释放池。 */
    @Test
    ...
    /** 音乐交给全局播放器并携带 Feed 队列，不创建页面级播放器。 */
    @Test
